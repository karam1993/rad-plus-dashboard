<template>
  <div>
    <v-expansion-panels v-if="filters.status" v-permissions="permissions.showAny" class="mb-2">
      <v-expansion-panel>
        <v-expansion-panel-header><h3 class="primary--text">{{ $t('common.filter') }}</h3></v-expansion-panel-header>
    ...
    >
      ...
      <template v-slot:[`item.workingStatus`]="{ item }">
        <div>
          ...
          <span :title="$t('cards.itNotWorking')" v-else><v-icon color="error">mdi-wifi-off</v-icon></span>
