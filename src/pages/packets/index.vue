...
export default {
    ...
    return {
      ...
      packetFilter : {
        ...
        items : [
          ...
          { placeHolder : this.$t('packets.type') , key: 'filter[type]' , value : '' , type : 'autocomplete' , items: [{ name : 'ppp' , value : 'ppp' } , { name : 'hotspot' , value : 'hotspot' }] , itemText : 'name' , itemId : 'value' }  
        ]
      },
      packetPagination:{
        pagesCounts : 1 ,
  ...
  methods: {
    ...
    goToAttribute(item) {
      this.$router.push('/packets/' + item.packetid + '/attribute')
    }
