    ...
    getSessions(query = `?page[number]=${this.sessionPagination.selectedPage}`) {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          this.sessionItems.map((item) => (
            item.acctstarttime = item.acctstarttime !== '' ? dateFormat(item.acctstarttime, 'HH:MM dd-mm-yyyy') : '' ,
            item.acctstoptime = item.acctstoptime !== '' ? dateFormat(item.acctstoptime, 'HH:MM dd-mm-yyyy') : ''
          ))
